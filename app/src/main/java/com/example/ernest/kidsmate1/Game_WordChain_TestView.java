...
public class Game_WordChain_TestView extends AppCompatActivity {
    // 디버깅 메시지
    protected static final String TAG = Game_WordChain_TestView.class.getSimpleName();
    ...
    protected void showTotalResult(){
        ...
        game_result.setOnCancelListener(new DialogInterface.OnCancelListener(){
            ...
            public void onCancel(DialogInterface dialog){
                Game_WordChain_TestView.this.finish();
            }
        });
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView_word = gameView.getQuizWordChain();       // custom view 테스트중 17.05.09
    ...
    protected static class InnerEventHandler extends Handler {
        // 이벤트 핸들러 이너 클래스
        private final WeakReference<Game_WordChain_TestView> mActivity;
        InnerEventHandler(Game_WordChain_TestView activity) {
            mActivity = new WeakReference(activity);
        }
        @Override
        public void handleMessage(Message msg) {
            Game_WordChain_TestView activity = mActivity.get();
            if (activity != null) {
